const CLAVE_ALMACEN = 'loyola_usuarios';
...
export const servicioUsuarios = {
  obtenerTodos: async (filtros = {}) => {
    ...
    if (filtros.termino) {
      ...
      datos = datos.filter(d => ['nombre', 'email', 'rol'].some(f => d[f]?.toLowerCase().includes(t)));
    }
    return datos;
  },
  ...
  buscar: async (termino) => {
    return servicioUsuarios.obtenerTodos({ termino });
  },
  contarTodos: async () => {
