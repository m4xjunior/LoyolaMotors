const CLAVE_ALMACEN = 'loyola_vehiculos';
...
export const servicioVehiculos = {
  obtenerTodos: async (filtros = {}) => {
    let datos = obtenerAlmacen();
    if (filtros.termino) {
      const t = filtros.termino.toLowerCase();
      datos = datos.filter(d => ['marca', 'modelo', 'matricula'].some(f => d[f]?.toLowerCase().includes(t)));
    }
    return datos;
  },
  ...
  buscar: async (termino) => {
    return servicioVehiculos.obtenerTodos({ termino });
  },
  contarTodos: async () => {
