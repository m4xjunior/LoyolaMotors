const CLAVE_ALMACEN = 'loyola_citas';
...
export const servicioCitas = {
  obtenerTodos: async (filtros = {}) => {
    ...
    if (filtros.termino) {
      ...
      datos = datos.filter(d => ['clienteNombre', 'descripcion'].some(f => d[f]?.toLowerCase().includes(t)));
    }
    return datos;
  },
  ...
  buscar: async (termino) => {
    return servicioCitas.obtenerTodos({ termino });
  },
  contarTodos: async () => {
