const CLAVE_ALMACEN = 'loyola_facturas';
...
export const servicioFacturas = {
  obtenerTodos: async (filtros = {}) => {
    ...
    if (filtros.termino) {
      ...
      datos = datos.filter(d => ['numero', 'clienteNombre'].some(f => d[f]?.toLowerCase().includes(t)));
    }
    return datos;
  },
  ...
  buscar: async (termino) => {
    return servicioFacturas.obtenerTodos({ termino });
  },
  contarTodos: async () => {
